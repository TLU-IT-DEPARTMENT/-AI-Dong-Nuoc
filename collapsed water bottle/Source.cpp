...
struct State
{
	...
	State(int x, int y, int oper = -1) : _x(x), _y(y), _o(oper), _p(-1) {} // constructor 
	inline State create(int oper) const // inline function
	{
		switch (oper)
		{
		case 0: // x --> 0
...
int main()
{
	...
	cout << "Nhap dung tich binh thu nhat : "; cin >> x;
	cout << "Nhap dung tich binh thu hai : "; cin >> y;
	cout << "Nhap dung tich can dong : "; cin >> z;
	cout << endl;
	...
	int iRet = BFS(x, y, z);
	if (-1 != iRet)
		cout << "Sau " << iRet << " buoc, ta lay duoc " << z << " lit. :D" << endl;
	else
		cout << "No way ! :(" << endl;
	cout.flush();
